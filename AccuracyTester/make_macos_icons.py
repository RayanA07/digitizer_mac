"""Generate AccuracyTester/assets/accuracytester.icns from a source image.

Looks for a source image in this folder's ``assets`` directory (a PNG or an ICO,
e.g. the Windows ``accuracytester.ico``) and converts it to a macOS ``.icns``.
...
ASSETS = ROOT / "assets"
TARGET = ASSETS / "accuracytester.icns"

SOURCE_CANDIDATES = [
    ASSETS / "accuracytester.png",
    ASSETS / "icon.png",
    ASSETS / "accuracytester.ico",
    ASSETS / "icon.ico",
]
